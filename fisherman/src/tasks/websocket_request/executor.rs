...
use common::tasks::websocket_request::{
    JobWebsocketResponse, JobWebsocketResponseDetail, JobWebsocketResult,
};
...
impl WebsocketRequestExecutor {
    ...
    pub async fn call_websocket_request(
        ...
        if let Some(JobDetail::Websocket(request)) = &job.job_detail {
            ...
            log::trace!("Request header {:?}", &headers);

            if let Some(body) = &request.body {
                let mut client = ClientBuilder::new(job.component_url.as_str())
                    .unwrap()
                ...
                let message = Message::text(req_body);
                //println!("{:?}", &message);
                let result = client.send_message(&message);
                let response = if let Ok(OwnedMessage::Text(mess)) = client.recv_message() {
                    debug!("Socket message {:?}", &mess);
                ...
                Err(HttpRequestError::BuildError("Invalid job body".to_string()))
            }
        } else {
            Err(HttpRequestError::BuildError(
                "Invalid job detail".to_string(),
            ))
        }
    }
}
...
impl TaskExecutor for WebsocketRequestExecutor {
    async fn execute(&self, job: &Job, result_sender: Sender<JobResult>) -> Result<(), Error> {
        ...
        let res = self.call_websocket_request(job).await;
...
mod tests {
    ...
    fn new_test_job(component_url: &str, job_detail: &str) -> Job {
        ...
        let mut job_websocket: JobWebsocket = serde_json::from_str(job_detail).unwrap();
        ...
        let mut job = Job::new(
            ...
            "".to_string(),
            &component,
            JobDetail::Websocket(job_websocket),
            Default::default(),
        );
    ...
    #[tokio::test]
    async fn test_call_websocket_request() {
        ...
        match response.detail {
            JobWebsocketResponseDetail::Body(_) => {
                assert_eq!(0, 1);
            }
