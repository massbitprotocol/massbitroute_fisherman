...
use websocket::header::{Headers, Host};
use websocket::native_tls::{TlsConnector, TlsConnectorBuilder};
use websocket::sync::Client;
use websocket::url::{Position, Url};
            ...
            .map(|builder| builder.custom_headers(&headers))
        {
            ...
            let received_message = if request.url.starts_with("wss") {
                ...
                let mut client = client_builder.connect_insecure().map_err(|err| {
                    HttpRequestError::SendError(format!("Can not connect to {:?}", &request.url))
                })?;
    ...
    async fn test_eth_gateway_websocket() {
        ...
        for provider in providers.iter() {
            ...
            match response.detail {
                JobWebsocketResponseDetail::Body(body) => {
